    ...
    <DB as DrawingBackend>::ErrorType: 'static,
{
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        let lines = wrap_label(&row.entry_name, CHARS_PER_LINE, max_lines);
...
fn wrap_label(name: &str, chars_per_line: usize, max_lines: usize) -> Vec<String> {
    if max_lines == 0 {
        return Vec::new();
    }
