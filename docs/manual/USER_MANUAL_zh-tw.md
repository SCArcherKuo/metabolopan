...
你看到的第一個畫面是 **Choose your analysis**，它提供兩條路線。
選定其中一條後，整個工作階段都會依循它——畫面、步進器與輸出都會隨之改變。

**路線 1 — 差異分析 + 富集分析**（三個階段）。比較兩組樣本，再檢定哪些 KEGG 路徑或模組在發生變化的代謝物中過度代表：
...
**路線 2 — KEGG 覆蓋率調查**（兩個階段）。把你偵測到的每一個代謝物對應到 KEGG 路徑或模組上，並報告每一個條目被涵蓋得多完整。**不執行任何統計檢定。** 它只需要一個 MS-DIAL `.txt`；群組 `.csv` 是選用的：

| 階段 | 輸入 | 關鍵操作 | 輸出 |
|-------|-------|---------------|--------|
| **第一階段 — 輸入** | MS-DIAL `.txt`（+ 選用的群組 `.csv`） | 同樣的解析與對齊 | 一張解析後的表格 |
| **第二階段 — 設定** | 解析後的表格 | 選用的逐群組存在性篩選、以 InChIKey 去除重複，接著對應到 KEGG | **一張覆蓋率表格 + 一張點圖** |

路線 2 的完整說明見 [KEGG 覆蓋率調查](#kegg-coverage-survey)。
除非某一節另有說明，本手冊其餘部分描述的都是路線 1。

> **注意：** 單模式跑一個 MS-DIAL `.txt`；雙模式跑兩個 MS-DIAL `.txt` 檔（正離子 + 負離子）。在路線 1 上兩者都需要群組 `.csv`，且雙模式還需要其中的 `biosample` 欄。你所選的模式在第一階段設定，並貫穿後續每一個階段。
...
> **注意：核心概念 — 缺失值與真正的零。** 空白或 `"NA"` 儲存格會變成內部的「缺失」標記（`f64::NAN`），下游統計會**跳過**它；而寫成 `0` 的則是真正的零，會**參與**運算。詳見完整的 [缺失值 vs 真正的零](#missing-values-nan-vs-true-zeros-00) 章節。
  ...
  - [KEGG 覆蓋率調查](#kegg-coverage-survey)
    - [何時使用](#when-to-use-it)
    - [輸入](#inputs)
    - [設定畫面](#setup-screen)
    - [結果畫面](#result-screen)
...
<a id="kegg-coverage-survey"></a>
## KEGG 覆蓋率調查（KEGG coverage survey）

覆蓋率調查回答的問題與富集分析不同：不是*「哪些路徑改變了？」*，而是*「每一條路徑我究竟看到了多少？」*
它取用你偵測到的每一個代謝物，將其對應到 KEGG 的路徑或模組目錄上，並針對每一個條目報告：你偵測到該條目中的多少個化合物、以及那佔多少比例。

**它不執行任何統計檢定。** 這條路線上沒有 *p* 值、沒有 *q* 值、沒有 FDR 方法、沒有富集比、也沒有任何顯著性門檻。
這是刻意的設計決策而非疏漏，且它由輸入本身推導而來：超幾何檢定需要一個從*背景*中抽出的*前景*，而在沒有兩組比較的情況下，並不存在一個站得住腳的前景可抽。
對著一個憑空發明的前景報告 *p* 值，只會給出一個看起來像證據、實則不是的數字。

> **⚠ 警告：** 高覆蓋率百分比同時反映**生物學與你的方法能偵測到什麼**。
> 一個化合物剛好易於游離、能在你的梯度中沖提出來、且落在你的譜庫中的條目，會比一個同樣存在但化合物不具這些條件的條目得到更高的分數。
> 覆蓋率是關於你的資料集觸及範圍的陳述，而非關於某條路徑生物學重要性的陳述。
> 結果畫面會在視窗內重複這一點，匯出的 CSV 也會在第二行再重複一次。
...
如果你**確實**有兩組、每組至少兩個樣本，而且想知道哪些路徑*改變了*，請改用路線 1——見 [第三階段 — 富集分析](#stage-3--enrichment-over-representation-analysis)。

<a id="inputs"></a>
### 輸入（Inputs）

- **MS-DIAL `.txt`** — 必要，單模式或雙模式皆可，與路線 1 完全相同。
...
> **注意：** 此篩選讀取的是原始的、如載入時的強度矩陣，絕不讀取正規化後的矩陣。覆蓋率路線不提供樣本正規化。

> **⚠ 警告 — gap-filling。** MS-DIAL 對於未偵測到的位置會寫入字面上的 `0`，而上述存在性檢定把 `0` 視為不存在。**若你在 MS-DIAL 中啟用了 gap-filling**，未偵測到的位置改為帶有小的非零值，此檢定就會把它們算作*存在*。
> 因此，一份經過 gap-filling 的資料集，會比同一份未經 gap-filling 匯出的資料集報告出更高的覆蓋率。若你需要較嚴格的讀數，請關閉 gap-filling 重新從 MS-DIAL 匯出。
...
**Analysis Mode + 目標** — 與富集分析設定畫面完全相同：Pathway / Module 單選、物種選擇器或 Level + Group 挑選器，以及行內的 KEGG 取得進度。

**Run Coverage** — 在 KEGG 取得進行中或尚未選定目標時停用；另外，當已載入 `.csv` 且你取消勾選了所有群組時也會停用（滑鼠提示：`Select at least one sample group.`）。

<a id="result-screen"></a>
### 結果畫面（Result screen）

**來源漏斗（provenance funnel）** 位於畫面最上方，依各篩選階段實際執行的順序，每個階段一項：
...
未提供 `.csv` 時，`in selected groups` 這一項會被完全省略——該階段沒有執行，而在那裡重複原始計數等於印出一個從未發生的步驟。
...
| --- | --- |
| `Entry` | KEGG 路徑或模組 ID |
| `Name` | 條目名稱；滑鼠停留可看完整文字 |
| `Hits` | `<偵測到> / <條目大小>` — **永遠兩個數字都顯示** |
| `Coverage` | `hits / entry_size` 以百分比表示，一位小數 |
| `Compounds` | 偵測到的 KEGG 化合物 ID，以 `…(+N)` 標記截斷；完整清單在 CSV 中 |

`Hits` 欄絕不會只顯示一個沒有分母的計數，而 `Coverage` 也絕不會在同一列缺少 `Hits` 的情況下呈現。
一個脫離分母的覆蓋率百分比是這個畫面上最容易被誤讀的數字：一個含兩個化合物的條目的 `100 %` 與一個含五十個化合物的條目的 `40 %` 並不是可比較的主張，而那個分數正是讓這一點可見的東西。
...
| X 軸 | 富集倍率 | **你所選的排序依據** |
| 標記大小 | 命中數 | **條目大小**——恆定 |
| 標記顏色 | `-log10(FDR)` | **你*沒有*選作排序依據的那一項** |
| 參考線 | 在 `enrichment_ratio = 1` 處 | **無** |

**X 軸跟隨 Sort by 設定。** 選 `Coverage` 時，X 是覆蓋率 %、標記顏色是命中數；選 `Hits` 時兩者互換——X 變成命中數、顏色變成覆蓋率 %。
這兩個通道永遠一起移動：你用來排序的量就是列的上下順序所依據的量，把它放在 X 軸以外的地方，圖表的排序看起來就會像是任意的。
標記大小在兩種情況下都維持**條目大小**，因為它是另外兩個量共同的分母——整張圖中唯一固定的參照。
...
`Download coverage CSV` 會依顯示順序寫出**顯示中**的列，並在標題列之前放五行 `#` 註解：
...
`coverage` 與 `share` 是十進位**分數**，不是百分比。
`share` 是 `hits / detected_total`——「我的代謝體中有多少比例落在這個條目裡」，與 `coverage` 的「這個條目我偵測到了多少比例」是不同的問題。
它會被匯出但刻意**不顯示**：在畫面上，兩個無法互相比較的百分比只會誘使讀者去比較它們。

每一個命中化合物渲染為 `C00031 (D-Glucose)`——KEGG ID 後接**你自己的 MS-DIAL 代謝物名稱**，而非 KEGG 的化合物名稱，因此不會引入額外的查詢。
當一個化合物是從多個帶有不同名稱的特徵抵達時，全部名稱都會列出，排序後以 `" | "` 連接。
（用直線而非斜線：脂質簡寫如 `PC(16:0/18:1)` 本身就含有斜線，用 `/` 切分會把這些名稱切碎。）
當沒有可用的名稱時，寫出裸的 ID，不加括號。
